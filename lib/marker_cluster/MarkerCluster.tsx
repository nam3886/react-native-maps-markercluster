...
import { useMapViewContext } from "../useMapViewContext";
import { MarkerClusterItem } from "./MarkerClusterItem";

export const MarkerCluster: React.FC<MarkerClusterType.WrapperProps> = memo(
  (props) => {
    const { region, mapRef } = useMapViewContext();

    const [clusters, setClusters] = useState<MarkerClusterType.Cluster[]>([]);
